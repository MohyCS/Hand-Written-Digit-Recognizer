...
namespace prx
{
    ...
    namespace vis
    {
        ...
        {
            this->visualize_text_topic = node.subscribe("visualization/scene_text", 50, &scene_comm_t::text_callback, this);
        }
        ...
        bool scene_comm_t::rigid_bodies_callback(describe_geometries_srv::Request& request,
                                             describe_geometries_srv::Response& response)
        {
            PRX_DEBUG_S("CALL the geometry Callback");
            ...
            // PRX_INFO_S(geom_map.size());
            // foreach(geometry_info_t g, geom_map)
            // {
            //    PRX_INFO_S(g.print());
            // }
            // scene->add_geometry_batch(geom_map);
            scene->add_rigid_bodies(geom_map);
            return true;
        }
        ...
        {
            PRX_DEBUG_COLOR("Visualize obstacles callback", PRX_TEXT_CYAN);
        ...
        void scene_comm_t::text_callback(const prx_core::scene_text_msg& msg)
        {
