...
void osg_viewer_t::init_sky()
{
    ...
    // #else
    osg::ref_ptr< osg::Geode > skydome_geom = NULL;
    // #endif
...
//    boost::filesystem::create_directory( images_dir );
    ...
    osg::ref_ptr<osgViewer::ScreenCaptureHandler> screenShooter = new osgViewer::ScreenCaptureHandler( new osgViewer::ScreenCaptureHandler::WriteToFile( dir, "jpg", osgViewer::ScreenCaptureHandler::WriteToFile::OVERWRITE ) );
...
{
    // PRX_PRINT("Screenshot handlers..."<<screenshot_handlers.size()<<", "<<screen_num<<", "<<num_screenshots, PRX_TEXT_CYAN);
    if (screen_num < screenshot_handlers.size())
    {
        screenshot_handlers[follow_window_num]->setFramesToCapture(num_screenshots);
        screenshot_handlers[follow_window_num]->startCapture();
    }
}
...
{
    PRX_ERROR_S ("Change bounding box: " << selected_group);
    ...
    if (selected_group && render_bbox)
    {
        ...
        auto check1 = selected_group->asTransform();
        auto check2 = check1->asPositionAttitudeTransform();
        auto check3 = check2->addChild( osg_bbox );
...
void osg_viewer_t::update_follow_window()
{
    if(follow_name!="")
    {
        // PRX_STATUS("Following the object "<<follow_name, PRX_TEXT_MAGENTA);
        if(windows.size()>1)
        {
            // PRX_STATUS("Picking object "<<follow_name<<" on Window 1", PRX_TEXT_CYAN);
            osg::Group* pickedGroup = osg_scene->get_group_from_name(follow_name);
            // auto ptr = osg::get_pointer(osg_scene->get_group_from_name(follow_name));
            // if(ptr!=NULL)
            if(pickedGroup)
            {
                // osg::Group* pickedGroup = ptr->asGroup();
                // PRX_DEBUG_POINT("Check "<<pickedGroup);
                change_bounding_box(pickedGroup);
                handler->pick(follow_name);
                windows[follow_window_num]->follow_object();
                follow_name = "";
            }
        }
    }
...
void osg_viewer_t::deactivate_bounding_box()
{
    render_bbox = !render_bbox;
    // if bounding box has been toggled off
    if (!render_bbox)
...
void osg_viewer_t::init(const parameter_reader_t* reader)
{
    ...
    {
        if(key_value.first == "follow")
            follow_window_num = counter;
    ...
    */
    ...
    if(reader->has_attribute("follow_object"))
        follow_name = reader->get_attribute("follow_object");
}
