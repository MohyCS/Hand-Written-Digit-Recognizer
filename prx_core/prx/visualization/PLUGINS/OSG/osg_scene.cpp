    ...
    namespace vis
    {
        ...
        osg::ref_ptr<osg::Node> osg_scene_t::make_geometry(const std::string& material_name, const geometry_t* geom, osg_material_t* material, bool transparent, bool use_color)
        {
            ...
            if( transparent )
            {
                ...
                material->setTransparency(osg::Material::FRONT_AND_BACK, 0.4f);
            }
        ...
        void osg_scene_t::move_info_geometry(const std::string& name, const config_t& config)
        {
            ...
            // PRX_PRINT( "Setting: " << name << " : " << config.get_position(), PRX_TEXT_MAGENTA );
        }
        ...
        void osg_scene_t::draw_text(const scene_text_t& text_info)
        {
            ...
            else
            {
                ...
                scene_text_map[path]->setPosition(toVec3(text_info.relative_position));
        ...
        void osg_scene_t::init(const parameter_reader_t* reader)
        {
            ...
            const parameter_reader_t global_reader("", global_storage);
            ...
            osg::Vec4 ambient_vec(.50,.50,.50f,1.0f);
            osg::Vec4 diffuse_vec(.50,.50,.50f,1.0f);
            osg::Vec4 specular_vec(0.0,0.0,0.0,1.0f);

            myLight1->setLightNum(0);
            myLight1->setPosition(osg::Vec4(10.0,10.0,10.0,0.0f));
            myLight1->setAmbient(ambient_vec);
            myLight1->setDiffuse(diffuse_vec);
            myLight1->setSpecular(specular_vec);
            ...
            myLight2->setPosition(osg::Vec4(-10.0,10.0,10.0,0.0f));
            myLight2->setAmbient(ambient_vec);
            myLight2->setDiffuse(diffuse_vec);
            myLight2->setSpecular(specular_vec);
            // myLight2->setSpotCutoff(20.0f);
            // myLight2->setSpotExponent(50.0f);
            // myLight2->setDirection(osg::Vec3(-1.0f,-1.0f,0.0f));
            ...
            myLight3->setPosition(osg::Vec4(-10.0,-10.0,10.0,0.0f));
            myLight3->setAmbient(ambient_vec);
            myLight3->setDiffuse(diffuse_vec);
            myLight3->setSpecular(specular_vec);
            // myLight2->setSpotCutoff(20.0f);
            // myLight2->setSpotExponent(50.0f);
            // myLight3->setDirection(osg::Vec3(1.0f,1.0f,0.0f));
            osg::LightSource* lightS3 = new osg::LightSource;
            lightS3->setLight(myLight3);
            lightS3->setLocalStateSetModes(osg::StateAttribute::ON);
            lightS3->setStateSetModes(*(root->getOrCreateStateSet()),osg::StateAttribute::ON);
            lightGroup->addChild(lightS3);

            osg::Light* myLight4 = new osg::Light;
            myLight4->setLightNum(3);
            myLight4->setPosition(osg::Vec4(10.0,-10.0,10.0,0.0f));
            myLight4->setAmbient(ambient_vec);
            myLight4->setDiffuse(diffuse_vec);
            myLight4->setSpecular(specular_vec);
            // myLight2->setSpotCutoff(20.0f);
            // myLight2->setSpotExponent(50.0f);
            // myLight4->setDirection(osg::Vec3(1.0f,1.0f,0.0f));
            osg::LightSource* lightS4 = new osg::LightSource;
            lightS4->setLight(myLight4);
            lightS4->setLocalStateSetModes(osg::StateAttribute::ON);
            lightS4->setStateSetModes(*(root->getOrCreateStateSet()),osg::StateAttribute::ON);
            lightGroup->addChild(lightS4);

            // osg::ref_ptr<osg::StateSet> ss = lightGroup->getOrCreateStateSet();
            ...
            PRX_DEBUG_COLOR("Finished Creating the OSG scene.", PRX_TEXT_GREEN);
        }
            ...
            if( body_reader->has_element("visualization_geometry") )
            {
                ...
                if( body_reader->get_attribute("visualization_geometry/type") == "mesh" )
                {
                    ...
                    if( osg_visualization_geode != NULL )
                    {
                        ...
                        stateset=visualization_geode->getOrCreateStateSet (); 
                        auto material = new osg_material_t(*(template_material_mapping[visualization_material_name]));
                        material->setColorMode(osg::Material::OFF);
        ...
        osg::Group* osg_scene_t::get_group_from_name(std::string path)
        {
            auto ref_ptr = rigid_bodies[path];
            auto ptr = osg::get_pointer(ref_ptr);
            return ptr->asGroup();
        }
        ...
        void osg_scene_t::initialize_plants(const std::string& source_node, const std::vector<std::string>& paths, const std::vector<std::string>& template_paths)
        {
            ...
            bool global_reader = false;

            if(ros::param::has("prx/common_plant"))
            {
                global_reader = true;
                root_reader = new parameter_reader_t("prx/common_plant", global_storage);
            }

            sys_clock_t stats_clock;
            double string_manip_time = 0, root_reader_time = 0, geom_reader_time = 0;

            PRX_INFO_S("List of paths: " << paths.size());
            ...
            for( unsigned int i = 0; i < paths.size(); i++ )
            {
                ...
                string_manip_time += stats_clock.measure();
                ...
                stats_clock.reset();
                if(!global_reader)
                    root_reader = new parameter_reader_t(final_path, global_storage);
                ...
                if( root_reader->has_attribute("template") )
                {
                    ...
                    template_reader = new parameter_reader_t(source_node + "/" + template_namespace, global_storage);
                }
                root_reader_time += stats_clock.measure_reset();
                ...
                if( root_reader->has_attribute("geometries") )
                {
                    ...
                    PRX_PRINT("Adding group to systems at path: "<<paths[i], PRX_TEXT_BLUE);
                ...
                geom_reader_time += stats_clock.measure();
                if(!global_reader)
                    delete root_reader;
            }

            PRX_DEBUG_COLOR("Scene init times: ", PRX_TEXT_CYAN);
            PRX_DEBUG_COLOR("String manipulation: " << string_manip_time, PRX_TEXT_LIGHTGRAY);
            PRX_DEBUG_COLOR("Building the \'root\' readers: " << root_reader_time, PRX_TEXT_LIGHTGRAY);
            PRX_DEBUG_COLOR("Building geom readers: " << geom_reader_time, PRX_TEXT_LIGHTGRAY);

            //TODO: What is this optimizer and why aren't we using it?
            //            osgUtil::Optimizer t;
        ...
        void osg_scene_t::add_ghost_plant(const std::string& path, util::config_t rel_conf)
        {
            if( template_systems.find(path) != template_systems.end() )
            {
                ...
                if( root_plant != NULL )
                {
                    ...
                    copy_plant->getChild(0)->asTransform()->asPositionAttitudeTransform()->setAttitude(toOSGQuat(rel_conf.get_orientation()));
                    copy_plant->getChild(0)->asTransform()->asPositionAttitudeTransform()->setPosition(toVec3(rel_conf.get_position()));
        ...
        void osg_scene_t::add_rigid_bodies(const std::vector<util::geometry_info_t>& geoms)
        {
            ...
            for(auto& geom: geoms)
            {
                geom_name = geom.full_name;
                osg::ref_ptr<osg::PositionAttitudeTransform> parent = new osg::PositionAttitudeTransform();
                ///

                parent->setName(geom_name);
                osg::ref_ptr<osg::Node> collision_geode = make_geometry(geom.material_name, geom.geometry, geometry_material_mapping[geom_name], false);
                parent->addChild(collision_geode);
                root->addChild(parent);
                ///
                parent->setAttitude(toOSGQuat(root_conf.get_orientation()));
                parent->setPosition(toVec3(root_conf.get_position()));
                obstacles[geom_name] = parent;
            }
        }

        void osg_scene_t::visualize_obstacles(const std::string& obstacles_path)
        {
            PRX_INFO_S ("Looking for obstacles at: " << obstacles_path << "/obstacles");
            const parameter_reader_t global_reader("", global_storage);
            if( global_reader.has_element(obstacles_path + "/obstacles") )
            {

                PRX_INFO_S("Found obstacles in " << obstacles_path << "/obstacles");
                ...
                foreach(const parameter_reader_t::reader_map_t::value_type& body_key_value, global_reader.get_map(obstacles_path + "/obstacles"))
                {
                    ...
                    config_t root_conf;
                    root_conf.zero();
                    if(body_key_value.second->has_attribute("root_configuration"))
                        root_conf.init(body_key_value.second->get_child("root_configuration").get());
                    foreach(const parameter_reader_t* r, readers)
                    {
                        geom_name = r->get_attribute_as< std::string > ("name");
                        std::string name = obstacles_path + "/obstacles/" + body_key_value.first + "/" + geom_name;
                        // PRX_INFO_S("The geom name is : " << name << " | " << geom_name);
                        osg::ref_ptr<osg::PositionAttitudeTransform> parent = new osg::PositionAttitudeTransform();
                        init_geometries(obstacles_path + "/obstacles/" + body_key_value.first, geom_name, r, parent);
                        config_t conf;
                        conf.init(r->get_child("config").get());
                        // PRX_INFO_S("++["<<root_conf.get_position()[0]<<","<<root_conf.get_position()[1]<<","<<root_conf.get_position()[2]<<"]");
                        // PRX_INFO_S("--["<<conf.get_position()[0]<<","<<conf.get_position()[1]<<","<<conf.get_position()[2]<<"]");
                        conf.relative_to_global(root_conf);
