...
tree_edge_index_t tree_node_t::get_parent_edge() const
{
    return parent_edge;
}
...
unsigned tree_t::get_depth(tree_vertex_index_t v) 
{
    // PRX_INFO_S("Entering depth function");
...
void tree_t::remove_vertex(tree_vertex_index_t v)
{
    ...
    PRX_ASSERT(std::find(v_index_map[v_index_map[v]->parent]->children.begin(),v_index_map[v_index_map[v]->parent]->children.end(),v)!=v_index_map[v_index_map[v]->parent]->children.end() );
