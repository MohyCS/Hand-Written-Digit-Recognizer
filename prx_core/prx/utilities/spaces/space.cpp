...
#include "prx/utilities/math/quaternion.hpp"
#include "prx/utilities/math/angle.hpp"
...
namespace prx
{
    namespace util
    {
        space_t* space_t::space_from_topology(const std::string& name, const std::string& topo, const std::vector<double*>& addresses) 
        {
            space_t* new_space = new space_t();

            new_space->dimension = addresses.size();
            PRX_DEBUG_S("SPACES " << name << "    dimension: " << new_space->dimension);
            new_space->local_offset = 0;

            new_space->space_name = name;
            new_space->quaternion = false;
            new_space->set_topology_string(topo);

            PRX_ASSERT(new_space->topology.size() == addresses.size());

            new_space->addresses.clear();

            foreach(double* val, addresses)
            {
                new_space->addresses.push_back(val);
            }


            for( unsigned i = 0; i < new_space->dimension; i++ )
            {
                new_space->bounds.push_back(new bounds_t());
                new_space->scale.push_back(new double);
            }
            new_space->set_default_bounds();
            new_space->set_default_scale();

            new_space->owned_bounds = true;
            return new_space; 
        }
        ...
        space_t::space_t(const std::string& name, const std::vector<double*>& addresses)
        {
            ...
            //is populated by the file: "prx_root/prx_input/templates/spaces/space_types.yaml"
            init_space_types();
            ...
            std::string topo;
            if( space_topologies.find(name)!=space_topologies.end())
                topo = space_topologies[name];
            else
            {
                ...
                {
                    if( space_topologies.find(elem)!=space_topologies.end())
                        topo +=  space_topologies[elem];
                    else
        ...
        void space_t::init(const parameter_reader_t * const reader)
        {
            ...
            {
                PRX_PRINT("Setting scale...", PRX_TEXT_CYAN);
        ...
        bool space_t::equal_points(const space_point_t * const point1, const space_point_t * const point2, double epsilon) const
        {
            ...
            for( unsigned i = 0; i < dimension; ++i )
            {
                ...
                {
                    bool ret = true;
                    for( unsigned int q_i=0; q_i<4; q_i++ )
                        if( fabs( point1->memory[i+q_i] - point2->memory[i+q_i] ) > epsilon)
                            ret = false;
                ...
                {
                    if( fabs(point1->memory[i] - point2->memory[i] ) > epsilon )
                    {
                        return false;
                    }
                }
        ...
        void space_t::enforce_bounds(space_point_t* point, const std::vector<bounds_t*>* additional_bounds) const
        {
            ...
            for( unsigned int i = 0; i < size; ++i )
            {
                ...
                else if( topology[i] == ROTATIONAL )
                {
                    ...
                    //     p -= 2 * PRX_PI;
                    // }
                    p  = (*additional_bounds)[i]->adjust(p);
        ...
        double space_t::distance(const space_point_t * const point1, const space_point_t * const point2) const
        {
            ...
            for( unsigned int i = 0; i < dimension; ++i )
            {
                ...
                else if( topology[i] == QUATERNION && (*scale[i]) > PRX_ZERO_CHECK )
                {
                    ...
                    double average_scale = ((*scale[i] + *scale[i+1] + *scale[i+2])/3.0);
                    double test_dist = q1.distance(q2);
#ifndef RELEASE
                    test_dist = (test_dist < 1e-7 ? 0.0 : test_dist);
#endif
                    // PRX_DEBUG_COLOR("Average scale: " << average_scale, PRX_TEXT_CYAN);
                    // PRX_DEBUG_COLOR("QUATERNION DIST: " << test_dist, PRX_TEXT_CYAN);
                    ret += (average_scale*test_dist);
                    i += 3;
                }
                else if( topology[i] == DISCRETE )
        ...
        void space_t::uniform_sample(space_point_t * const point) const
        {
            ...
            for( unsigned int i = 0; i < dimension; ++i )
            {
                if( topology[i] == QUATERNION )
                {
                    ...
                    q.random();
                    q.get(point->memory[i], point->memory[i + 1], point->memory[i + 2], point->memory[i + 3]);
                    //ZL 4/24/17: make sure that the quaternion satisfies bounds
                    point->memory[i]=bounds[i]->adjust(point->memory[i]);
                    point->memory[i+1]=bounds[i+1]->adjust(point->memory[i+1]);
                    point->memory[i+2]=bounds[i+2]->adjust(point->memory[i+2]);
                    point->memory[i+3]=bounds[i+3]->adjust(point->memory[i+3]);
                    q.set(point->memory[i], point->memory[i + 1], point->memory[i + 2], point->memory[i + 3]);
                    q.rectify();
                    q.get(point->memory[i], point->memory[i + 1], point->memory[i + 2], point->memory[i + 3]);   
                    ////////////////////////                 
        ...
        void space_t::uniform_sample_ball(space_point_t * const point, double radius) const
        {
            ...
            std::vector<double> offsets(dimension);
            while( true )
            {
                double ss = 0;
                for( unsigned i = 0; i < dimension; i++ )
                {
                    offsets[i] = gaussian_random();
                    ss += pow(offsets[i], 2.0);
                }
                double norm = sqrt(ss);
        ...
        void space_t::set_scales(const std::vector<double>& inscales)
        {
            ...
            for( unsigned i = local_offset; i < dimension; i++ )
            {
                *(this->scale[i]) = inscales[i - local_offset];
                // PRX_PRINT("Set scale [" << i << "]: " << *(this->scale[i]), PRX_TEXT_MAGENTA);
            }
        }
        ...
        void space_t::scale_bounds(double scaling_factor)
        {
            ...
            for( unsigned int i = 0; i < dimension; ++i )
            {
                if( topology[i] != DISCRETE )
                {
                    bounds[i]->scale_bound(scaling_factor);
                }
            }
        }
        ...
        void space_t::is_compatible(const space_point_t * const point) const
        {
            ...
            {
                PRX_PRINT("Spaces are incompatible! Parent of passed point: " << point->parent->space_name.c_str() << " Space name: " << space_name.c_str(), PRX_TEXT_BLUE);
                PRX_DEBUG_POINT("boop");
