...
namespace prx
{
    namespace util
    {
        ...
        bool quaternion_t::is_approximate_equal( const quaternion_t& other, double tol ) const
        {
            bool ret = true;
            for( unsigned int i=0; i<4; i++ )
                if( fabs( q[i] - other.q[i] ) > tol)
                    ret = false;
        ...
        {
            return fabs(norm()-1.000000) < PRX_DISTANCE_CHECK;
        ...
        double quaternion_t::distance( const quaternion_t& other ) const
        {
            ...
            const double lambda = fabs((q[0]*other.q[0]) + (q[1]*other.q[1]) + (q[2]*other.q[2]) + (q[3]*other.q[3]));
            return fabs(1 - fabs(lambda));
        }

        double quaternion_t::slerp_distance( const quaternion_t& other ) const
        {
            PRX_ASSERT_MSG(is_valid(),
                           "Invalid quaternion: %f %f %f %f", q[0], q[1], q[2], q[3]);
            PRX_ASSERT_MSG(other.is_valid(),
            ...
            vector_t v1(q[0],q[1],q[2],q[3]);
            vector_t v2(other.q[0], other.q[1], other.q[2], other.q[3]);
            const double lambda = v1.dot_product(v2);
            ...
            double theta = 2 * std::asin(d/2.0);

            // Normalizing to make rang from [-pi/2, pi/2] to [-1,1]
            theta /= (PRX_PI);
        ...
        {
            // PRX_DEBUG_COLOR("V1: " << v1, PRX_TEXT_CYAN);
            // PRX_DEBUG_COLOR("V2: " << v2, PRX_TEXT_GREEN);
            ...
            double dot_prod = v1.dot_product(v2);
            if (dot_prod > (-1 + PRX_ZERO_CHECK) )
            {
            ...
            else
            {
                ...
                if (fabs(v2[0]) > PRX_ZERO_CHECK)
                {
                    temp_v.set(((-2*v2[1] - 2*v2[2])/v2[0]), 2, 2);
                }
                else if (fabs(v2[1]) > PRX_ZERO_CHECK)
                {
                    temp_v.set(2, ((-2*v2[0] - 2*v2[2])/v2[1]), 2);
                }
                else if (fabs(v2[2]) > PRX_ZERO_CHECK)
                {
                    temp_v.set(2,2, ((-2*v2[0] - 2*v2[1])/v2[2]));
                }
                else
                {
                    PRX_ERROR_S ("Attempted to compute rotation for an invalid vector! : " << v2);
                }
            ...
            //PRX_PRINT("PRE NORMALIZE Quaternion: " << (*this), PRX_TEXT_MAGENTA);
            //PRX_PRINT("PRE NORMALIZE dot : " << v1.dot_product(v2) << " and sqrt: " << sqrt(v1.squared_norm() * v2.squared_norm()), PRX_TEXT_MAGENTA);
            //PRX_PRINT("POST NORMALIZE Quaternion: " << (*this), PRX_TEXT_MAGENTA);
