...
{ 
    namespace util 
    {
         ...
         *
         */
        vector_t::vector_t( const std::vector<double>& other )
        {
            _vec.resize( other.size() );
         ...
         *
         */
        vector_t& vector_t::operator= ( const std::vector<double>& input_vec )
        {
            _vec.resize(input_vec.size());
         ...
         *
         */
        bool vector_t::is_approximate_equal( const vector_t& other, double tol ) const
        {
            for( unsigned int i=0; i<_vec.size(); i++ )
                if( fabs( _vec[i] - other._vec[i] ) > tol )
                    return false;
            return true;
        }
