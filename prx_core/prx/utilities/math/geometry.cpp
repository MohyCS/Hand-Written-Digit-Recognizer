...
#include "prx/utilities/math/geometry.hpp"
...
namespace prx
{
    namespace util
    {
        ...
        void geometry_t::init_geometry(std::string type, vector_t& dims)
        {
            ...
            {
                if (dims.get_dim() != 3)
                    PRX_FATAL_S("Ellipsoid must have three-dimensional dims attribute.")
                    set_ellipsoid(dims[0],dims[1],dims[2]);
            }
            else
            {
        ...
        void geometry_t::set_ellipsoid(double a, double b, double c)
        {
            type = PRX_ELLIPSOID;
            info.clear();
            info.push_back(a);
            info.push_back(b);
            info.push_back(c);
            trimesh.create_ellipsoid_trimesh(a,b,c);
        }
        ...
        void geometry_t::get_ellipsoid(double& a,double& b,double& c) const
        {
            PRX_ASSERT(type == PRX_ELLIPSOID);
            a = info[0];
            b = info[1];
            c = info[2];
        }
        ...
        void geometry_t::set_params( const std::vector<double>* params )
        {
            switch (type)
            {
                ...
                case PRX_ELLIPSOID:
                    ...
                    set_ellipsoid((*params)[0], (*params)[1], (*params)[2]);
                    break;
        ...
        double geometry_t::get_bounding_radius() const
        {
            ...
            switch (type)
            {
                case PRX_SPHERE:
                    ...
                    break;
                case PRX_ELLIPSOID:
                    bounding_radius = PRX_MAXIMUM(PRX_MAXIMUM(info[1],info[2]),info[0]);
                    break;
        ...
        void geometry_t::init(const parameter_reader_t* const reader)
        {
            ...
            {
                const vector_t dims = reader->get_attribute_as<vector_t>("radii");
                if (dims.get_dim() != 3)
                    PRX_FATAL_S("Ellipsoid must have three-dimensional dims attribute.")
                    set_ellipsoid(dims[0], dims[1], dims[2]);
            }
        ...
        //     clpr.AddPaths(simple_subj, ClipperLib::ptSubject, true);
        //     clpr.AddPaths(simple_clip, ClipperLib::ptClip, true);
