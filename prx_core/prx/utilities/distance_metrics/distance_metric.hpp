    ...
    namespace util 
    {
        ...
        class distance_metric_t
        {
          ...
          public:
             ...
             */
            virtual const abstract_node_t* single_query( const space_point_t* query_point, double* dist = NULL ) const = 0;
            ...
            {
                return single_query(query_point->point,dist);
            }
            ...
            virtual void link_distance_function( distance_function_t* input_function );

            virtual distance_function_t* get_distance_function();

            /**
             * Query to check whether the point exists in the metric, and return it.
             * @brief Query to find the point in the metric.
             * @param query_point The state to query.
             * @return The found node to the query point, and NULL if the point does not exist.
             */
            virtual const abstract_node_t* find_query( space_point_t* query_point );
            virtual const abstract_node_t* find_state_query( space_point_t* query_point );
             ...
             * @return The found node to the query point, and NULL if the point does not exist.
             */
            virtual const abstract_node_t* find_query( const abstract_node_t* query_node );
          ...
          protected:
            ...
            /**
             * @brief The precision up to which the state vector doubles are considered.
             */
            int precision;

            /**
             * @brief Convert the double in to a specific precision.
             */
            double get_precision( double in, int in_precision );
            
            /**
             * @brief Get the long int version of the double in, up to a precision
             */
            long int get_rounded_double(double in);
            
            /**
             * @brief Given a vector of doubles, create a vector of long int using desired precision
             */
            std::vector<long int> get_key(std::vector<double> in);
            
            /**
             * @brief The node point is added to the map using the key, and the node is the value.
             */
            void add_point_to_map( const abstract_node_t* node);

            /**
             * @brief The map used for resolving find queries. The key is the converted state vector, and the value is the abstract node in the metric.
             */
            std::map< std::vector< long int >, const abstract_node_t* > find_query_map;
