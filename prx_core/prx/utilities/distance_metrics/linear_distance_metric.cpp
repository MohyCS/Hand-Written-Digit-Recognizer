...
#include "prx/utilities/distance_metrics/linear_distance_metric.hpp"
...
namespace prx
{
    ...
    {       
        ...
        const abstract_node_t* linear_distance_metric_t::single_query(const space_point_t* query_point, double* dist) const
        {
            double min_distance = std::numeric_limits<double>::max();
            int min_index = -1;
            for( unsigned i = 0; i < nr_points; i++ )
